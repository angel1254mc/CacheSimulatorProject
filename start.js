...
for (let i = 0; i < 6; i++) {
    ...
    let d_map_cache = new DirectMapCache(256*Math.pow(2,i), 1, 8, Math.pow(2, 32));
    let full_assoc_cache_FIFO = new FullyAssociativeCache(256*Math.pow(2,i), 1, 8, Math.pow(2,32), "FIFO");
    let full_assoc_cache_LRU = new FullyAssociativeCache(256*Math.pow(2,i), 1, 8, Math.pow(2,32), "LRU");
    let two_set_associative_FIFO = new NSetCache(128*Math.pow(2,i), 2, 8, Math.pow(2, 32), "FIFO");
    let two_set_associative_LRU = new NSetCache(128*Math.pow(2,i), 2, 8, Math.pow(2, 32), "LRU");
    let four_set_associative_FIFO = new NSetCache(64*Math.pow(2,i), 4, 8, Math.pow(2, 32), "FIFO");
    let four_set_associative_LRU = new NSetCache(64*Math.pow(2,i), 4, 8, Math.pow(2, 32), "LRU");
    let eight_set_associative_FIFO = new NSetCache(32*Math.pow(2,i), 8, 8, Math.pow(2, 32), "FIFO");
    let eight_set_associative_LRU = new NSetCache(32*Math.pow(2,i), 8, 8, Math.pow(2, 32), "LRU");
    ...
    console.log(`Simulation finalized for Cache Size ${Math.pow(2,i)*256*4}`);

    resultsArray.push({
        cache_size: Math.pow(2,i)*256*4,
        data: [
